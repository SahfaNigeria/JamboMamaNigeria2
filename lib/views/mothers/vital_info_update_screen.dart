...
class _VitalInfoUpdateScreenState extends State<VitalInfoUpdateScreen> {
  ...
  // Input formatters to allow only numbers and decimal points
  final _decimalInputFormatter = [
    FilteringTextInputFormatter.allow(RegExp(r'^\d*\.?\d*')),
  ];

  final _integerInputFormatter = [
    FilteringTextInputFormatter.digitsOnly,
  ];

  // Validator to block special characters
  String? _validateNumericInput(String? value, {bool allowDecimal = false}) {
    if (value == null || value.isEmpty) {
      return null; // Allow empty as fields are optional
    }

    // Check for special characters
    final specialCharsRegex = RegExp(r'[!@#$%^&*()_+=\[\]{};:"\\|,<>/?`~]');
    if (specialCharsRegex.hasMatch(value)) {
      return autoI8lnGen.translate('NO_SPECIAL_CHARS');
    }

    // Validate numeric format
    if (allowDecimal) {
      if (double.tryParse(value) == null) {
        return autoI8lnGen.translate('INVALID_NUMBER');
      }
    } else {
  ...
  Future<void> _submitVitalInfo() async {
    ...
    try {
      ...
      final providerIds =
          query.docs.map((doc) => doc['recipientId'] as String).toList();

      // STEP 2: Prepare data — store providerIds array instead of single providerId
      final vitalInfoData = {
        'userId': widget.userId,
        'providerIds': providerIds, // ← array, not single string
      ...
      // STEP 3: Save to GLOBAL vital_info collection (scoped under patient)
      await _firestore
          .collection('vital_info') // global collection
          .doc(widget.userId) // patient as document
          .collection('records') // subcollection of records
          .add(vitalInfoData);

      // STEP 4: Save under EACH connected provider's subcollection
      ...
      // STEP 5: Update latest info for patient (unchanged)
      await _firestore.collection('New Mothers').doc(widget.userId).set({
  ...
  // }
  String _getHaemoglobinGuidance(double? hb) {
    if (hb == null) return ''; // ← empty, not a warning
    if (hb < 10.5) return autoI8lnGen.translate("R_W_G_9");
    if (hb <= 12) return autoI8lnGen.translate("R_W_G_10");
    if (hb > 14) return autoI8lnGen.translate("R_W_G_11");
    return '';
  ...
  String _getBloodPressureGuidance(int? systolic, int? diastolic) {
    if (systolic == null || diastolic == null) return '';
    // Severe — checked first so it isn't swallowed by the >= 140 branch
            ...
            children: [
              ...
              _buildCard(
                ...
                child: TextFormField(
                  ...
                  validator: (value) =>
                      _validateNumericInput(value, allowDecimal: true),
                  decoration: InputDecoration(
                    hintText: autoI8lnGen.translate("E_C_W"),
                    errorMaxLines: 2,
                  ),
                ),
              ...
              _buildCard(
                ...
                child: TextFormField(
                  ...
                  validator: (value) =>
                      _validateNumericInput(value, allowDecimal: true),
                  decoration: InputDecoration(
                    hintText: autoI8lnGen.translate("EN_H"),
                    errorMaxLines: 2,
                  ),
                ),
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        ...
                        validator: (value) =>
                            _validateNumericInput(value, allowDecimal: false),
                        decoration: InputDecoration(
